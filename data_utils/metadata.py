...
class PreprocessingInfo:
    ...
    def add_step(
        self, step_name: str, parameters: Dict[str, Any], description: str = ""
    ):
...
class DatasetCompatibility:
    ...
    def add_model_compatibility(
        self, model_type: str, batch_size: int, notes: str = ""
    ):
...
class DatasetMetadata:
    ...
    def add_split_info(
        self, split_name: str, n_samples: int, split_ratio: float = None, **kwargs
    ):
    ...
    def add_validation_result(
        self, test_name: str, result: Any, passed: bool, details: str = ""
    ):
    ...
    def calculate_quality_score(self) -> float:  # noqa: C901
        ...
        if self.statistics:
            ...
            if self.statistics.missing_values == 0:
                ...
                missing_ratio = self.statistics.missing_values / (
                    self.shape.n_samples if self.shape else 1
                )
            ...
            if self.statistics.class_balance:
                ...
                if balance_values:
                    balance_score = 1.0 - np.std(
                        balance_values
                    )  # Higher score for balanced classes
                    score += balance_score * 15

            # Low outliers bonus
            if hasattr(self.statistics, "outliers_detected"):
                outlier_ratio = self.statistics.outliers_detected / (
                    self.shape.n_samples if self.shape else 1
                )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DatasetMetadata":
        ...
        if "compatibility" in data and data["compatibility"]:
            data["compatibility"] = DatasetCompatibility.from_dict(
                data["compatibility"]
            )
    ...
    def _validate_data_consistency(self, metadata: DatasetMetadata, results: Dict):
        ...
        if metadata.splits and metadata.shape:
            ...
            if total_split_samples > metadata.shape.n_samples:
                results["warnings"].append("Total split samples exceed dataset size")
...
def create_metadata_from_data(
    ...
    **kwargs,
    ...
    statistics = DatasetStatistics(
        ...
        target_distribution={
            str(float(val)): int(count) for val, count in zip(unique_values, counts)
        },
        class_balance={},
    )
    ...
    compatibility = DatasetCompatibility(
        frameworks=["numpy", "pytorch"], min_memory_mb=X.nbytes / (1024 * 1024)
    )
    ...
    logger.debug(
        f"Created metadata for {name}: {shape.n_samples} samples, "
...
def save_metadata_to_file(
    metadata: DatasetMetadata, file_path: Union[str, Path]
) -> None:
    ...
    if not validation_results["valid"]:
        logger = get_logger(__name__)
        logger.warning(
            f"Saving metadata with validation errors: {validation_results['errors']}"
        )
...
def load_metadata_from_file(
    ...
    if validate:
        ...
        if not validation_results["valid"]:
            logger = get_logger(__name__)
            logger.warning(
                f"Loaded metadata has validation issues: {validation_results['errors']}"
            )
