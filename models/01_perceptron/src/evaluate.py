...
def print_evaluation_summary(results, dataset_config: dict, experiment_name: str):
    ...
    if experiment_name in ['iris_binary', 'linear_separable', 'debug_small', 'debug_linear']:
        if actual_acc >= 0.9:
            print("✓ This result demonstrates Perceptron's strength on linearly separable data")
...
def main():
    ...
    try:
        ...
        if args.output or evaluation_config.save_results:
            output_path = args.output or f"outputs/evaluations/{args.experiment}_{args.split}_evaluation.json"
        ...
        if args.visualize:
            ...
            try:
                ...
                if model_config['input_size'] == 2:
                    boundary_path = plots_dir / f"{args.experiment}_{args.split}_decision_boundary.png"
        ...
        if args.save_predictions and hasattr(results, 'predictions'):
            ...
            pred_data = {
                ...
                'probabilities': results.probabilities if hasattr(results, 'probabilities') else None
            }
