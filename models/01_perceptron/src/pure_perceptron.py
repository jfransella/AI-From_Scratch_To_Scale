...
class PurePerceptron:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, verbose: bool = False) -> Dict[str, Any]:
        ...
        if X.shape[1] != self.input_size:
            raise ValueError(f"X must have {self.input_size} features, got {X.shape[1]}")
        ...
        self.logger.info("Training perceptron on %d samples with %d features", len(X), X.shape[1])
