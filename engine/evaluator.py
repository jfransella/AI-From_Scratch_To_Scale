"""
Model evaluation engine for AI From Scratch to Scale project.

This module provides comprehensive evaluation capabilities including metrics
computation, confusion matrices, and detailed performance analysis for
models across the project.
"""
...
class Evaluator:
    ...
    def _compute_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> ModelMetrics:
        ...
        if self.config.compute_per_class and metrics.num_classes > 1:
            try:
                ...
                metrics.per_class_precision = {f"class_{i}": p for i, p in enumerate(precisions)}
                
                # Recall per class
                recalls = recall_score(y_true, y_pred, average=None, zero_division=0)
                metrics.per_class_recall = {f"class_{i}": r for i, r in enumerate(recalls)}
                ...
                if metrics.confusion_matrix is not None:
                    ...
                    for i in range(len(unique_labels)):
                        if i < metrics.confusion_matrix.shape[0]:
                            ...
                            per_class_acc[f"class_{i}"] = class_correct / class_total if class_total > 0 else 0.0
    ...
    def compare_models(self, 
                      ...
                      dataset_name: str = "unknown") -> Dict[str, EvaluationResult]:
        """
        ...
        if self.config.verbose:
            ...
            for name, result in results.items():
                ...
                print(f"{name:<15} {result.accuracy:<10.4f} {result.loss:<10.6f} {f1_str:<10}")
