...
class DatasetMetadata:
    ...
    def calculate_quality_score(self) -> float:
        ...
        info_score = 0
        if self.description: info_score += 5
        if self.source: info_score += 5
        if self.shape: info_score += 5
        if self.statistics: info_score += 5
        score += info_score
        ...
        doc_score = 0
        if self.preprocessing and self.preprocessing.steps: doc_score += 3
        if self.compatibility: doc_score += 3
        if self.license: doc_score += 3
        if self.citation: doc_score += 3
        if len(self.splits) > 1: doc_score += 3  # Multiple splits documented
        score += doc_score
