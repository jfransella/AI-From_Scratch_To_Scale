...
class TestPerceptronTrainingPipeline:
    ...
    def test_data_loading_pipeline(self):
        ...
        assert torch.all((unique_targets == 0) | (unique_targets == 1)), "Targets should be 0 or 1"
    ...
    def test_model_save_load_integration(self):
        ...
        assert torch.allclose(original_output, loaded_output), "Outputs should be identical"
...
class TestPerceptronEvaluationPipeline:
    ...
    def test_evaluation_metrics_computation(self):
        ...
        # Verify metrics
        assert 0 <= accuracy.item() <= 1, "Accuracy should be between 0 and 1"
        assert probabilities.shape == (100, 2), "Probabilities should have correct shape"
        assert torch.allclose(probabilities.sum(dim=1), torch.ones(100)), "Probabilities should sum to 1"
