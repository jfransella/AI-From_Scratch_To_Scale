...
class DatasetCache:
    ...
    def save_to_cache(self, data: Dict[str, np.ndarray], category: str,
        ...
        try:
            ...
            self.logger.debug("Cached data to %s (%.2f MB)", cache_path, cache_entry.get_size_mb())
    ...
    def load_from_cache(self, category: str, cache_key: str,
        ...
        try:
            ...
            self.logger.debug("Loaded from cache: %s (%.2f MB)", cache_key, cache_entry.get_size_mb())
    ...
    def cleanup_expired_entries(self, force: bool = False) -> Dict[str, Any]:
        ...
        self.logger.info("Cache cleanup completed: %d expired, %d corrupted, %.2f MB freed",
                         stats['expired_removed'], stats['corrupted_removed'],
            ...
            if cache_file.is_file() and not cache_file.name.startswith('.'):
                ...
                try:
                    ...
                    try:
                        ...
                        stats['corrupted'] += 1
                        stats['space_freed_mb'] += size_mb
                        self.logger.debug("Removed corrupted cache: %s", cache_file.name)
                    except Exception:
                        pass
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        current_stats.update({
            ...
            'usage_percentage': (float(current_stats['total_size_mb']) / 1024) / float(self.config['max_cache_size_gb']) * 100
        })
    ...
    def clear_cache(self, category: Optional[str] = None, confirm: bool = False) -> Dict[str, Any]:
        ...
        self.logger.info("Cache cleared: %d entries, %.2f MB freed",
                         stats['entries_removed'], stats['space_freed_mb'])
