...
class CacheEntry:
    ...
    def __init__(self, cache_path: Path, category: str, key: str, 
...
class DatasetCache:
    ...
    def _initialize_cache_structure(self) -> None:
        ...
        try:
            ...
            for category, category_config in CACHE_CATEGORIES.items():
                subdir = self.base_dir / category_config['subdir']
                subdir.mkdir(parents=True, exist_ok=True)
            
            self.logger.debug(f"Initialized cache structure at {self.base_dir}")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize cache structure: {e}")
            raise DataError(f"Cache initialization failed: {e}")
    
    def _load_cache_metadata(self) -> Dict[str, Any]:
        """Load cache metadata from disk."""
        metadata_path = self.base_dir / self.config['metadata_filename']
    ...
    def generate_cache_key(self, category: str, identifier: str, 
    ...
    def get_cache_path(self, category: str, cache_key: str, 
        ...
        cache_dir = self.base_dir / category_config['subdir']
    ...
    def save_to_cache(self, data: Dict[str, np.ndarray], category: str, 
                     cache_key: str, metadata: Optional[Dict[str, Any]] = None) -> CacheEntry:
        """
        ...
        try:
            ...
            self.logger.debug(f"Cached data to {cache_path} ({cache_entry.get_size_mb():.2f} MB)")
            ...
            raise DataError(f"Cache save failed: {e}")
    
    def load_from_cache(self, category: str, cache_key: str, 
                       validate_ttl: bool = True) -> Optional[Tuple[Dict[str, np.ndarray], CacheEntry]]:
        """
        ...
        if validate_ttl:
            category_config = CACHE_CATEGORIES[category]
            if not cache_entry.is_valid(category_config['ttl_hours']):
        ...
        try:
            ...
            self.logger.debug(f"Loaded from cache: {cache_key} ({cache_entry.get_size_mb():.2f} MB)")
            
            return data, cache_entry
            
        except Exception as e:
            self.logger.warning(f"Failed to load from cache {cache_key}: {e}")
    ...
    def _update_cache_metadata(self, cache_entry: CacheEntry, operation: str) -> None:
        ...
        try:
            if operation == 'add':
                ...
                self.metadata['total_size_mb'] = max(0.0, 
                    self.metadata['total_size_mb'] - cache_entry.get_size_mb())
                ...
                if cache_entry.category in self.metadata['categories']:
                    ...
                    cat_stats['size_mb'] = max(0.0, cat_stats['size_mb'] - cache_entry.get_size_mb())
    ...
    def _should_cleanup(self) -> bool:
        ...
        try:
            ...
            return hours_since >= self.config['cleanup_frequency_hours']
        except Exception:
            return True
    ...
    def _check_cleanup_schedule(self) -> None:
        ...
        if self._should_cleanup():
            try:
                ...
                self.logger.warning(f"Scheduled cleanup failed: {e}")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        current_stats.update({
            ...
            'usage_percentage': (current_stats['total_size_mb'] / 1024) / self.config['max_cache_size_gb'] * 100
        })
        
        return current_stats
    
    def clear_cache(self, category: Optional[str] = None, confirm: bool = False) -> Dict[str, Any]:
        """
        Clear cache entries.
        ...
        if category:
            ...
            cache_dir = self.base_dir / CACHE_CATEGORIES[category]['subdir']
            if cache_dir.exists():
            ...
            for cat_name, cat_config in CACHE_CATEGORIES.items():
                cache_dir = self.base_dir / cat_config['subdir']
                if cache_dir.exists():
                    removed_count, space_freed = self._clear_directory(cache_dir)
        ...
        self.logger.info(f"Cache cleared: {stats['entries_removed']} entries, "
                        f"{stats['space_freed_mb']:.2f} MB freed")
...
def cache_dataset(data: Dict[str, np.ndarray], category: str, identifier: str,
...
def load_cached_dataset(category: str, identifier: str,
                       params: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, np.ndarray]]:
    """
