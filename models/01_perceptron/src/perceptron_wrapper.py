...
class PerceptronWrapper(nn.Module, BaseModel):
    ...
    def _sync_pytorch_params(self):
        ...
        if hasattr(self.pure_perceptron, 'weights'):
            with torch.no_grad():
                ...
                self.linear.bias.data = torch.tensor([self.pure_perceptron.bias]).float()
    ...
    def demonstrate_learning_rule(self, X: np.ndarray, y: np.ndarray, max_steps: int = 5):
        """
        ...
        for i, (x, target) in enumerate(zip(X, y)):
            ...
            if prediction != target:
                ...
                # Apply update
                self.pure_perceptron.weights += self.pure_perceptron.learning_rate * error * x
                self.pure_perceptron.bias += self.pure_perceptron.learning_rate * error
                
                print(f"  ❌ Wrong! Applying update...")
                print(f"  Error: {error}")
                print(f"  Weight update: {old_weights} + {self.pure_perceptron.learning_rate} × {error} × {x}")
                print(f"                 = {self.pure_perceptron.weights}")
                print(f"  Bias update: {old_bias:.3f} + {self.pure_perceptron.learning_rate} × {error}")
