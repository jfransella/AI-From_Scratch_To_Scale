"""
Standard setup_path.py for 02_adaline model.

This module ensures that shared packages (utils, engine, data_utils, plotting)
can be imported properly in the ADALINE model directory. This fixes editable
installation issues by manually adding the project root to Python path.

Usage:
    # At the top of any script in the adaline directory:
...
def setup_shared_packages_path():
    ...
    # Get the current file's directory (models/02_adaline/)
    current_dir = Path(__file__).parent.absolute()
    
    # Go up two levels to reach project root
    # models/02_adaline/ -> models/ -> project_root/
