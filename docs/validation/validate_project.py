...
class ProjectValidator:
    ...
    def validate_model_structure(self, model_dir: Path):
        ...
        required_structure = {
            ...
            "notebooks": ["01_Theory_and_Intuition.ipynb", "02_Code_Walkthrough.ipynb", "03_Empirical_Analysis.ipynb"],
    ...
    def validate_model_config(self, model_dir: Path):
        ...
        try:
            ...
            if hasattr(config_module, 'get_config'):
                ...
                # Try to get a config
                try:
                    config = config_module.get_config('debug_small')
                    self.add_result(
                        "config_validation",
                        ValidationLevel.SUCCESS,
    ...
    def add_result(self, check_name: str, level: ValidationLevel, message: str, 
                   details: Optional[Dict] = None, file_path: Optional[Path] = None):
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        help="Validate specific model directory"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Validate all models and project structure"
    )
    parser.add_argument(
        "--check",
    ...
    if args.all:
        ...
        if args.check == "structure":
            ...
            logger.error(f"Unknown check type: {args.check}")
            sys.exit(1)
    else:
