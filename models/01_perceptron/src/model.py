...
from constants import (
    ...
    DEFAULT_ACTIVATION, DEFAULT_INIT_METHOD
)
...
class Perceptron(nn.Module, BaseModel):
    ...
    def __init__(self, 
        ...
        self.logger.info(f"Perceptron initialized: {input_size} inputs, {activation} activation")
                ...
                if prediction != y[i]:
                    ...
                    with torch.no_grad():
                        if y[i] == 1:  # Misclassified positive
                            self.linear.weight.data += self.learning_rate * X[i].unsqueeze(0)
