...
# Import Perceptron model for testing
try:
    from model import Perceptron
except ImportError:
    # Mock Perceptron for testing when model module is not available
    Perceptron = Mock()
...
class TestPrepareEvaluationData:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.dataset_config = {"dataset_name": "test_dataset", "dataset_params": {}}

        # Create sample data
        self.sample_x = torch.randn(100, 4)
        self.sample_y = torch.randint(0, 2, (100,)).float()

    def test_prepare_test_split(self):
        """Test preparing test split data."""
        # Patch the module directly
        with (
            patch.object(evaluate_module, "get_logger", return_value=Mock()),
            patch.object(
                evaluate_module,
                "load_dataset",
                return_value=(self.sample_x, self.sample_y),
            ),
        ):
    ...
    def test_prepare_full_split(self):
    ...
    def test_tensor_conversion(self):
        ...
        with (
            patch.object(evaluate_module, "get_logger", return_value=Mock()),
            patch.object(
                evaluate_module, "load_dataset", return_value=(x_numpy, y_numpy)
            ),
        ):
